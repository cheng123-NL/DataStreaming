...
KAFKA_BROKERS = "localhost:19092, localhost:39092, localhost:49092" 
NUM_PARTITIONS = 5
    ...
    try:
        ...
        if topic_name not in metadata.topics:
            ...
            for topic, future in fs.items():
                try:
                    future.result()
                    logger.info(f"Topic '{topic_name} created susscessfully!")
...
def generate_transaction():
    return dict(
        ...
        amout = round(random.uniform(50000, 150000), 2),
        transationTime= int(time.time()),
        ...
        currency = random.choice(['USD','EU', 'GBP'])
    )
